...
class OutputGuardrailPlugin(base_plugin.BasePlugin):
    ...
    def _replace_response_text(self, llm_response, new_text: str):
        """Replace llm_response content text in a model-safe way."""
    ...
    async def after_model_callback(
        ...
        *,
        ...
        evaluated_text = response_text

        if not filter_result["safe"]:
            self.redacted_count += 1
            evaluated_text = filter_result["redacted"]
            llm_response = self._replace_response_text(llm_response, evaluated_text)
        ...
        if self.use_llm_judge:
            ...
            if not judge_result["safe"]:
                ...
                safe_message = (
                    ...
                    "Please ask a banking-related question in a safe format."
                )
                llm_response = self._replace_response_text(llm_response, safe_message)
